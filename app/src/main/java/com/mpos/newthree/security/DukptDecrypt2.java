...
public class DukptDecrypt2 {
    ...
    public static String decrypt(String ksn, String bdk, String data) throws Exception {
        byte[] ksnbytes = Utils.hex2byte(ksn);
        byte[] databytes = Utils.hex2byte(data);
        byte[] bdkbytes = Utils.hex2byte(bdk);
        byte[] ipek = generateIPEK(ksnbytes, bdkbytes);
        byte[] datakey = getDatekey(ksnbytes, ipek);
        return Utils.hexString(tdesDec(databytes, datakey));
    }

    private static byte[] getDatekey(byte[] ksn, byte[] ipek) throws Exception {
        byte[] key = Utils.trim(ipek, 16);
        byte[] cnt = new byte[3];
    ...
    private static byte[] NRKGP(byte[] key, byte[] ksn) throws Exception {
        byte[] key_temp = Utils.trim(key, 8);
        byte[] temp = new byte[8];
        for (int i = 0; i < 8; i++) {
    ...
    public static byte[] generateIPEK(byte[] ksn, byte[] bdkbytes) throws Exception{
        byte[] temp = Utils.trim(ksn, 8);
        byte[] keyTemp = bdkbytes;
        temp[7] = (byte) (0xE0 & temp[7]);
        byte[] temp2 = tdesEnc(temp, keyTemp);
        byte[] result = Utils.trim(temp2, 8);
        keyTemp[0] ^= 0xC0;
        keyTemp[1] ^= 0xC0;
        keyTemp[2] ^= 0xC0;
        keyTemp[3] ^= 0xC0;
        ...
        result = Utils.concat(result, Utils.trim(temp2, 8));
        return result;
    }
    ...
    private static byte[] tdesEnc(byte[] data, byte[] key) throws Exception {
        ...
        if (key.length == 8) {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(Utils.concat(key, 0, 16, key, 0, 8), ALG_TRIPLE_DES));
        }
    ...
    private static byte[] tdesDec(byte[] data, byte[] key) throws Exception {
        ...
        if (key.length == 8) {
            ...
            k = new SecretKeySpec(Utils.concat(key, 0, 16, key, 0, 8), ALG_TRIPLE_DES);
        }
